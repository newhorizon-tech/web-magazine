describe "the article creation process", type: :feature do
  ...
  it "gives an error when submitting a blank article with no attributes" do
    ...
    expected_msg = "Image can't be blank, Title can't be blank, Title is too short (minimum is 3 characters), and At least one category must be selected"
  ...
  it "gives an error when submitting an article with short title" do
    ...
    click_button 'Create Article'
    msg = find('div.flash.lead')
    expected_msg = "Image can't be blank, Title is too short (minimum is 3 characters), and At least one category must be selected"
    expect(msg).to have_content expected_msg
  end
  ...
  it "gives an error when submitting an article with no categories" do
    ...
    click_button 'Create Article'
    msg = find('div.flash.lead')
    expected_msg = "Image can't be blank and At least one category must be selected"
    expect(msg).to have_content expected_msg
  end
end
